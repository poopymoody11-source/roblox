...
--------------------------------------------------------------------------
-- A NAME PLATE: who's talking. Returns remove() (fades it out).
--------------------------------------------------------------------------
function Kit.nameTag(parent, name, yOffset, size, color, z)
	local K = Kit
	color = color or Color3.fromRGB(200, 185, 255)
	local spaced = string.upper(name):gsub(".", "%0 "):sub(1, -2)
	local tag = Instance.new("TextLabel")
	tag.BackgroundTransparency = 1
	tag.AnchorPoint = Vector2.new(0.5, 1)
	tag.Position = UDim2.new(0.5, 0, 0.5, yOffset)
	tag.Size = UDim2.fromOffset(size * #spaced * 0.9 + 40, size * 1.4)
	tag.FontFace = K.Fonts.Title
	tag.TextSize = size
	tag.Text = spaced
	...
	st.Thickness = math.max(1.5, size / 10)
	...
	bar.Position = UDim2.new(0.5, 0, 0.5, yOffset + size * 0.15)
	bar.Size = UDim2.fromOffset(0, math.max(2, size / 9))
	bar.ZIndex = z or 80
	bar.Parent = parent
	local g = Instance.new("UIGradient")
	g.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.2, 0), NumberSequenceKeypoint.new(0.8, 0), NumberSequenceKeypoint.new(1, 1) })
	g.Parent = bar
	tag.Position = UDim2.new(0.5, -size * 2, 0.5, yOffset)
	TweenService:Create(tag, TweenInfo.new(0.3, Enum.EasingStyle.Quint), { TextTransparency = 0, Position = UDim2.new(0.5, 0, 0.5, yOffset) }):Play()
	TweenService:Create(st, TweenInfo.new(0.3), { Transparency = 0.15 }):Play()
	TweenService:Create(bar, TweenInfo.new(0.4, Enum.EasingStyle.Quint), { Size = UDim2.fromOffset(size * #spaced * 0.75 + 30, math.max(2, size / 9)) }):Play()
	return function()
		TweenService:Create(tag, TweenInfo.new(0.25), { TextTransparency = 1 }):Play()
...
function Kit.say(text, hold, style)
	...
	-- who's talking (style.Speaker)
	local untag = style.Speaker and Kit.nameTag(holder, style.Speaker, top - size * 0.72, math.floor(size * 0.34), style.SpeakerColor, 63)
	-- slam in
