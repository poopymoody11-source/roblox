...
--==================================================
local Ch = {}
local Dodge = require(script.Parent:WaitForChild("Dodge"))
...
-- a dodged obstacle is thrown aside (the way opposite to the dodge)
local function dodgeShift(ctx, c, T)
	if not c or not c.Dodged or not c.DodgeVec then return Vector3.zero end
	local u = smooth((T - c.DodgeT) / 0.45)
	local b = ctx.SOL.ByKey[c.Key]
	local R = (b and b.R) or 200
	local mag = c.Meteor and 650 or c.Rings and R * 5 or R * 1.3 + 700
	return c.DodgeVec * mag * u
end
...
local function crashPull(ctx, key, pos, T)
	...
	return pos:Lerp(target, u) + dodgeShift(ctx, cr, T)
end
...
-- (bright enough that every world's night side still shows its surface)
local AMB0, OAMB0 = Color3.fromRGB(62, 62, 76), Color3.fromRGB(92, 92, 108)
local function setAura(ctx, a)
	local L = game:GetService("Lighting")
	L.Ambient = AMB0:Lerp(Color3.fromRGB(105, 68, 150), a)
	L.OutdoorAmbient = OAMB0:Lerp(Color3.fromRGB(135, 92, 190), a)
...
function Ch.Space(ctx, t0, dur)
	...
	K.run(t0, dur, function(t, dt)
		...
		cue("dodgeIntro", t >= 19.2, function() Dodge.intro(ctx) end)
...
local function cseq(...)
	local a = { ... }
	local kps = {}
	for i = 1, #a, 2 do table.insert(kps, ColorSequenceKeypoint.new(a[i], a[i + 1])) end
	return ColorSequence.new(kps)
end

-- burning up: a re-entry plasma sheath streaming back from the crown of the head,
-- flames and embers torn off the body, a glowing streak, and the glow of it
local function addBurn(ctx, rig)
	local K = ctx.kit
	if rig.Burn or not rig.Torso then return end
	local torso = rig.Torso
	local head = rig.Model:FindFirstChild("Head") or torso
	local bn = { Items = {}, Amt = -1 }
	rig.Burn = bn
	local function att(parent, pos)
		...
		a.Position = pos
		a.Parent = parent
		table.insert(bn.Items, a)
		return a
	end
	-- (the body's +Y is the way it's diving: head first)
	local front = att(head, Vector3.new(0, 1.6, 0))
	local back = att(torso, Vector3.new(0, -17, 0))
	local function sheath(w0, w1, col, bright, speed, t0)
		local b = Instance.new("Beam")
		b.Attachment0 = front
		b.Attachment1 = back
		b.FaceCamera = true
		...
		b.Segments = 10
		b.Color = col
		b.Transparency = K.ns(0, t0, 0.35, t0 + 0.2, 1, 1)
		b.Parent = torso
		table.insert(bn.Items, b)
		return b
	end
	bn.Outer = sheath(6, 1.2, cseq(0, Color3.fromRGB(255, 190, 90), 0.4, Color3.fromRGB(255, 90, 40), 1, Color3.fromRGB(150, 60, 255)), 1.6, 3, 0.35)
	bn.Inner = sheath(2.4, 0.3, cseq(0, Color3.fromRGB(255, 250, 225), 0.5, Color3.fromRGB(255, 170, 80), 1, Color3.fromRGB(255, 90, 60)), 2.6, 5, 0.25)
	bn.Fire = K.emitter(torso, {
		Texture = FIRE, Color = cseq(0, Color3.fromRGB(255, 240, 200), 0.3, Color3.fromRGB(255, 140, 50), 1, Color3.fromRGB(150, 60, 255)),
		Size = K.ns(0, 1.8, 1, 0.3), Transparency = K.ns(0, 0.3, 0.6, 0.6, 1, 1), Lifetime = NumberRange.new(0.18, 0.34),
		Speed = NumberRange.new(28, 46), SpreadAngle = Vector2.new(16, 16), EmissionDirection = Enum.NormalId.Bottom,
		LockedToPart = true, Brightness = 3, Rotation = NumberRange.new(0, 360), RotSpeed = NumberRange.new(-220, 220),
	})
	bn.Embers = K.emitter(torso, {
		Texture = SPARK, Color = cseq(0, Color3.fromRGB(255, 245, 210), 1, Color3.fromRGB(255, 120, 50)),
		Size = K.ns(0, 0.28, 1, 0), Lifetime = NumberRange.new(0.25, 0.55), Speed = NumberRange.new(40, 90),
		SpreadAngle = Vector2.new(28, 28), EmissionDirection = Enum.NormalId.Bottom, LockedToPart = true, Brightness = 4,
	})
	local cap = att(head, Vector3.new(0, 1, 0))
	bn.Cap = K.emitter(cap, {
		Texture = FIRE, Color = cseq(0, Color3.fromRGB(255, 250, 230), 1, Color3.fromRGB(255, 150, 70)),
		Size = K.ns(0, 3.6, 1, 2), Transparency = K.ns(0, 0.35, 1, 1), Lifetime = NumberRange.new(0.08, 0.14),
		Speed = NumberRange.new(0, 0), LockedToPart = true, Brightness = 3, Rotation = NumberRange.new(0, 360),
	})
	for _, e in ipairs({ bn.Fire, bn.Embers, bn.Cap }) do table.insert(bn.Items, e) end
	local l0 = att(torso, Vector3.new(-1.1, 0, 0))
	local l1 = att(torso, Vector3.new(1.1, 0, 0))
	local tr = Instance.new("Trail")
	tr.Attachment0 = l0
	tr.Attachment1 = l1
	tr.Lifetime = 0.25
	...
	tr.LightEmission = 1
	tr.Brightness = 2
	tr.Color = cseq(0, Color3.fromRGB(255, 200, 120), 0.4, Color3.fromRGB(255, 100, 60), 1, Color3.fromRGB(140, 60, 255))
	tr.Transparency = K.ns(0, 0.2, 1, 1)
	tr.WidthScale = K.ns(0, 1, 1, 0.1)
	tr.Parent = torso
	table.insert(bn.Items, tr)
	bn.Trail = tr
	...
	pl.Shadows = false
	pl.Parent = torso
	table.insert(bn.Items, pl)
	bn.Light = pl
end

local function burnUpdate(rig, amt)
	local bn = rig.Burn
	if not bn then return end
	amt = math.clamp(amt, 0, 1.5)
	if math.abs(amt - bn.Amt) < 0.03 then return end
	bn.Amt = amt
	bn.Fire.Rate = 30 + 110 * amt
	bn.Embers.Rate = 10 + 70 * amt
	bn.Cap.Rate = 25 * amt
	bn.Outer.Width0 = 2 + 4.5 * amt
	bn.Outer.Width1 = 0.4 + 1.2 * amt
	bn.Inner.Width0 = 0.8 + 2 * amt
	bn.Outer.Brightness = 0.6 + 1.4 * amt
	bn.Inner.Brightness = 1 + 2 * amt
	bn.Outer.Enabled = amt > 0.05
	bn.Inner.Enabled = amt > 0.05
	bn.Light.Brightness = 1 + 3 * amt
	bn.Trail.Enabled = amt > 0.1
end

local function burnOff(rig)
	local bn = rig.Burn
	if not bn then return end
	rig.Burn = nil
	for _, d in ipairs(bn.Items) do
		if d:IsA("ParticleEmitter") or d:IsA("Beam") or d:IsA("Trail") or d:IsA("PointLight") then d.Enabled = false end
	end
	task.delay(0.7, function() for _, d in ipairs(bn.Items) do pcall(function() d:Destroy() end) end end)
end

--------------------------------------------------------------------------
-- the flight (DRAG and VORTEX share it): a head-first dive at the portal.
-- Layered: the dive pose shifting between arms-forward and arms-swept-back,
-- wind rippling the limbs, the body banking into the turns of the path; a
-- brace before an impact, a full somersault and recovery after one, and a
-- barrel roll out of the way on a dodge
--------------------------------------------------------------------------
local function flyParty(ctx, T, t, wild, rush)
	local K = ctx.kit
	local SOL = ctx.SOL
	rush = rush or 0.6
	local f = progress(T)
	local p0, fwd, side, up = flightFrame(ctx, f)
	local _, fwd2 = flightFrame(ctx, math.min(f + 0.01, 1))
	local bank = math.clamp((fwd2 - fwd):Dot(side) * 40, -0.6, 0.6)
	local hit = T - (SOL.LastHitT or -99)
	local dodge = T - (SOL.DodgeT or -99)
	-- bracing for whatever's coming that they're not getting out of the way of
	local brace = 0
	for _, c in ipairs(CRASH) do
		if not c.Done and not c.Dodged then
			local dtc = c.T - T
			if dtc >= 0 and dtc < 0.55 and (c.QFail or not Dodge.active()) then brace = math.max(brace, smooth(1 - dtc / 0.55)) end
		end
	end
	local SUPER = { Neck = K.A(38, 0, 0), RS = K.A(125, 0, 14), LS = K.A(125, 0, -14), RH = K.A(-4, 0, 5), LH = K.A(4, 0, -5) }
	local TRACK = { Neck = K.A(32, 0, 0), RS = K.A(-40, 0, 20), LS = K.A(-40, 0, -20), RH = K.A(-6, 0, 3), LH = K.A(-6, 0, -3) }
	local SPREAD = { Neck = K.A(34, 0, 0), RS = K.A(35, 0, 95), LS = K.A(35, 0, -95), RH = K.A(-10, 0, 24), LH = K.A(-10, 0, -24) }
	local BRACE = { Root = K.A(-18, 0, 0), Neck = K.A(-30, 0, 0), RS = K.A(115, 0, 18), LS = K.A(115, 0, -18), RH = K.A(60, 0, 6), LH = K.A(45, 0, -6) }
	local TUCK = { Root = K.A(-10, 0, 0), Neck = K.A(-15, 0, 0), RS = K.A(20, 0, 14), LS = K.A(20, 0, -14), RH = K.A(70, 0, 4), LH = K.A(70, 0, -4) }
	local center
	for slot, rig in pairs(ctx.rigs) do
		local ph = slot * 1.7
		...
		-- a hit: knocked back and aside, then catching up
		local hk = 0
		if hit >= 0 and hit < 1.3 then
			hk = 1 - smooth(hit / 1.3)
			local kb = math.sin(math.min(hit / 0.9, 1) * pi)
			p = p - fwd * kb * 7 + (side * math.sin(hit * 9 + slot) + up * math.cos(hit * 7 + slot)) * hk * 2.5
		end
		-- a dodge: a swerve out the chosen way and back into line
		local dk = 0
		if dodge >= 0 and dodge < 0.8 and SOL.DodgeWorld then
			dk = math.sin(math.min(dodge / 0.8, 1) * pi)
			p = p + SOL.DodgeWorld * dk * 7
		end
		local body = CFrame.lookAt(p, p + fwd, up) * CFrame.Angles(math.rad(-90), 0, 0)
		-- roll about the dive axis: a lazy sway, banking into the turns, a barrel roll on a dodge
		local roll = math.sin(t * 0.7 + ph) * 0.22 + bank
		if dodge >= 0 and dodge < 0.7 then roll += 2 * pi * smooth(dodge / 0.65) * ((SOL.DodgeSign or 1) >= 0 and -1 or 1) end
		-- pitch about the shoulders: bobbing in the wind; a full somersault on a hit
		local pitch = math.sin(t * 1.3 + ph) * 0.06 * (0.5 + rush)
		local yawW = math.sin(t * 0.9 + ph * 2) * 0.05
		if hit >= 0 and hit < 1.3 then
			local flip = 1 - (1 - math.min(hit / 0.85, 1)) ^ 3
			pitch -= 2 * pi * flip
			yawW += math.sin(hit * 6) * 0.5 * hk
		end
		body = body * CFrame.Angles(0, roll, 0) * CFrame.Angles(pitch, 0, yawW)
		rig:setCF(body)
		-- the pose
		local w = smooth(0.5 + 0.5 * math.sin(t * 0.45 + ph))
		local pose = K.mixPose(SUPER, TRACK, w)
		pose = K.mixPose(pose, SPREAD, (1 - math.clamp(rush, 0, 1)) * 0.45)
		local amp = 3 + math.clamp(rush, 0, 1) * 7
		for i, key in ipairs({ "RS", "LS", "RH", "LH" }) do
			pose[key] = (pose[key] or CFrame.new()) * K.A(math.noise(t * 7, slot, i) * amp, 0, math.noise(t * 6, slot, i + 5) * amp * 0.6)
		end
		pose.RH = pose.RH * K.A(math.sin(t * 8 + ph) * 6, 0, 0)
		pose.LH = pose.LH * K.A(math.sin(t * 8 + ph + pi) * 6, 0, 0)
		pose.Neck = (pose.Neck or CFrame.new()) * K.A(0, math.sin(t * 0.6 + ph) * 12, 0)
		if brace > 0 then pose = K.mixPose(pose, BRACE, brace) end
		if dk > 0 then pose = K.mixPose(pose, TUCK, dk) end
		if hk > 0 then pose = K.mixPose(pose, K.flail(t, slot, 1.4, K.Poses.Blown), hk) end
		rig:setPose(K.safeArms(pose))
		rig:apply()
		if rig == ctx.myRig then center = p end
	end
...
-- the crashes: every target the party doesn't dodge blows apart as they smash
-- through it (and costs a hit); a dodged one is thrown aside and whips past
	...
	for _, c in ipairs(CRASH) do
		if c.Meteor then
			if not c.Done then
				...
				if c.Rock and (T < c.T or c.Dodged) then
					local hp, hf = flightFrame(ctx, progress(c.T))
					c.Rock.CFrame = CFrame.new(hp + hf * (c.T - T) * 1400 + dodgeShift(ctx, c, T)) * CFrame.Angles(T * 1.3, T * 0.9, 0)
				end
				if T >= c.T then
					if c.Dodged then
						if not c.Passed then
							c.Passed = true
							K.sfx(K.S.FireWhoosh, 0.8, 1.2)
							K.shake(1.2, 0.35)
						end
						if T >= c.T + 0.8 then
							c.Done = true
			...
			local hitNow = false
			if not c.Done and not c.Rings and not c.Dodged and T >= c.T - 3 then
			...
			if not c.Done and (T >= c.T or hitNow) then
				...
				if c.Dodged then
					-- a near miss: it roars past
					K.sfx(K.S.Whoosh, 1, 0.75)
					K.sfx(K.S.FireWhoosh, 0.6, 0.8)
					K.shake(1.8, 0.5)
				elseif b then
					SOL.LastHitT = T
					if b.Head then
...
-- where an obstacle is (to hang its dodge prompt over it)
local function obstaclePos(ctx, c)
	if c.Meteor then return c.Rock and c.Rock.Position, 130, "METEOR" end
	if c.Rings then return flightPoint(ctx, c.F) + dodgeShift(ctx, c, ctx.SOL.T or c.T), 1500, "SATURN'S RINGS" end
	local b = ctx.SOL.ByKey[c.Key]
	if not b then return nil end
	if b.Head then return b.Pos, 260, "COMET" end
	return b.Pos, b.R, string.upper(c.Key)
end
...
function Ch.Drag(ctx, t0, dur)
	...
	local cue = K.once()
	Dodge.start(ctx, CRASH, function() return K.now() - t0 + OFF end)
	...
	K.run(t0, dur, function(t, dt)
		...
		local f0 = progress(T)
		local speed = (flightPoint(ctx, progress(T + 0.05)) - flightPoint(ctx, f0)).Magnitude / 0.05
		local rush01 = math.clamp(speed / 2000, 0, 1)
		local center, fwd, side, up, f = flyParty(ctx, T, t, 1, rush01)
		doCrashes(ctx, T, fwd)
		SOL.T = T
		Dodge.update(T, function(c) return obstaclePos(ctx, c) end)

		-- burning up: hotter the faster they fall, flaring on every hit
		local hit = T - (SOL.LastHitT or -99)
		local flare = (hit >= 0 and hit < 0.6) and (1 - hit / 0.6) * 0.6 or 0
		local burnAmt = K.k(t, 0, 1.4) * (0.35 + rush01 * 0.65) + flare
		for _, rig in pairs(ctx.rigs) do burnUpdate(rig, burnAmt) end

		-- the chase: behind and a little above, rolling round the dive now and then;
		-- knocked back by every hit, leaning into every dodge
		local ang = chaseAngle(t)
		local cu = up * math.cos(ang) + side * math.sin(ang)
		local jolt = (hit >= 0 and hit < 0.8) and (1 - hit / 0.8) or 0
		local dist = 17 + math.sin(t * 0.6) * 3 + jolt * 8
		local camP = center - fwd * dist + cu * (5.5 + jolt * 3)
		local dodge = T - (SOL.DodgeT or -99)
		if dodge >= 0 and dodge < 0.8 and SOL.DodgeWorld then
			camP = camP - SOL.DodgeWorld * 2.5 * math.sin(math.min(dodge / 0.8, 1) * pi)
		end
		local camCF = CFrame.lookAt(camP, center + fwd * 140 + cu * 2, cu)
		local fov = 68 + rush01 * 10 + jolt * 8
		-- (flowing on from the last shot of SPACE: over their shoulders, the portal
		-- ahead, easing round into the chase as they're torn away)
		local blend = K.k(t, 0, 1.8, E.inOutSine)
		if blend < 1 then
			local toP = (SOL.Pp - center).Unit
			local startCF = CFrame.lookAt(center - toP * 16 + SOL.N * 4 + SOL.B * 8, center + toP * 800)
			camCF = startCF:Lerp(camCF, blend)
			fov = K.lerp(70, fov, blend)
		end
		K.setCam(camCF, fov)
		K.shake(0.25 + rush01 * 0.5, 0.1, 30, true)
		updateStreaks(ctx, camCF.Position, fwd, rush01 * K.k(t, 0.2, 1.2), t, true)
		rush.PlaybackSpeed = 0.9 + rush01 * 0.5
		cue("grab", t >= 0, function()
			K.sfx(K.S.Whoosh, 1, 0.6)
			K.sfx(K.S.DarkDrone, 0.7, 1.2)
			K.sfx(K.S.FireWhoosh, 0.8, 0.6)
			K.shake(2, 0.8)
		end)
		K.stream(center)
	end)
...
function Ch.Vortex(ctx, t0, dur)
	...
	for _, rig in pairs(ctx.rigs) do addBurn(ctx, rig) end
	...
	K.run(t0, dur, function(t, dt)
		...
		setAura(ctx, 1)
		K.starShellUpdate(stars, camPos, 0)
		local center, fwd, side, up = flyParty(ctx, T, t + DRAG, 1, 1)
		for _, rig in pairs(ctx.rigs) do burnUpdate(rig, 0.9 + e * 0.4) end
		doCrashes(ctx, T, fwd)
	...
	for _, rig in pairs(ctx.rigs) do burnOff(rig) end
	set.Parent = nil
end
