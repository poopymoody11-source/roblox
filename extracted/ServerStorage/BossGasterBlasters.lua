...
function GasterBlasters.new(boss, template)
	...
	-- Warm-up: the first time an AnimationId is ever loaded on a server, its keyframe data can
	-- take roughly a second to actually arrive -- Length reads 0 until then. If a real shot fires
	-- during that window it gets Played and Stopped with no pose data in between, and looks like
	-- the animation never played at all. Block here, once, until both are ready (or 5s passes as
	-- a safety cap) so every real shot in the fight has something to play. All 24 blasters share
	-- the same two Animation assets, so checking blaster 1 covers all of them.
	local first = self.Blasters[1]
	if first then
		local waited = 0
		while (first.PrepTrack.Length <= 0 or first.FireTrack.Length <= 0) and waited < 5 do
			task.wait(0.1)
			waited += 0.1
		end
	end
