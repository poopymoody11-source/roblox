...
------------------------------------------------------------------------
-- CRUSH: his fist closes on them - in three bites, each one tighter
------------------------------------------------------------------------
-- (the hold carries straight on from the end of the Catch: its clock is
-- the Catch's plus this; the Funnel's is this plus the Crush's)
local HOLD_T = 11
local BITES = { 0.35, 1.15, 1.95 }
...
function Ch.Crush(ctx, t0, dur)
	...
	-- a light on you in there, so you (and the edge of his grip) read against his black
	-- fist - it flares with every bite
	local lh = H.CrushLightHost
	if not lh then
		lh = K.part({ Name = "CrushLightHost", Size = Vector3.one, Transparency = 1, CFrame = CFrame.new(SK.CATCH) }, ctx.sets.Hand)
		local pl = Instance.new("PointLight")
		pl.Color = Color3.fromRGB(225, 215, 255)
		pl.Range = 16
		pl.Brightness = 0
		pl.Shadows = false
		pl.Parent = lh
		H.CrushLightHost, H.CrushLight = lh, pl
	end
	SK.run(K, t0, dur, function(t, dt)
		local T = t + HOLD_T
		local rootCF = holdRoot(ctx, T)
		ctx.BossRoot.CFrame = rootCF
		-- the bites: each one snaps a notch tighter (with a little overshoot, like a
		-- real grip biting in) and jolts everyone in it
		local bites, bump = 0, 0
		for _, at in ipairs(BITES) do
			bites += K.k(t, at, at + 0.28, K.E.outBack)
			if t >= at then bump = math.max(bump, math.exp(-(t - at) / 0.22)) end
		end
		local squeeze = bites / #BITES
		local crush = K.k(t, 3.6, 6.0, K.E.inOutSine)
		local lift = K.k(t, 0.2, 3.4, K.E.inOutSine)
		SB.reset()
		SB.body({
			-- (starts exactly as the Catch left him: bent over them, left arm hanging)
			Waist = K.A(K.lerp(-16, -14, lift) - bump * 1.5, 10 * lift, 0),
			LeftShoulder = K.A(-5, 0, K.lerp(-32, -30, lift)), LeftElbow = K.A(18, 0, 0),
		})
		SK.hang(SB, "Left", K.lerp(0.4, 0.5, lift), K.lerp(0.2, 0.1, lift))
		-- the fist lifts toward his face as it closes (and dips with each bite)
		local palm = SK.holdPalm(ctx, T, lift) - UP * bump * 1.2
		local hcf = SK.holdFist(ctx, SB, palm, rootCF)
		-- (and when the life starts going out of them, it squeezes harder still)
		local dying = K.k(t, 4.4, 6.6, K.E.inOutSine)
		local curl = SK.GRIP + bites * 0.06 + crush * 0.1 + dying * 0.12
		-- (the strain in his fingers is a slow shiver, not a buzz)
		SB.Hands.Right.H.pose(curl, 0.05 - squeeze * 0.2, 0.8 + squeeze * 0.2, 0.3 * (crush + dying), T * 0.3)
		local surf = CFrame.new(hcf:PointToWorldSpace(SK.fistSlot(ctx, 1)))
		local mine = hcf:PointToWorldSpace(SK.fistSlot(ctx, ctx.me or 1))
		local headCF = SB.R.cf("Head", rootCF)
		SB.lookAt(mine, 1, rootCF)
		holdGem(ctx, SB, rootCF, t)

		-- the party: shoving at his fingers, jolted by every bite, then crushed...
		-- going limp, head back
		local myPos = placeInFist(ctx, hcf, T, function(slot)
			local strain = K.lerp(0.4, 1, K.k(t, 0.2, 1.0)) * (1 - dying)
			local pose = SK.gripPose(K, T, slot, strain * (1 + crush * 0.6), dying)
			local jolt = bump * (1 - dying)
			-- (the Catch's head-up carries in, then each bite snaps the head back)
			pose.Neck = K.A(24 * (1 - K.k(t, 0, 0.8)), 0, 0) * (pose.Neck or CFrame.new())
				* K.A(jolt * 26 + crush * (1 - dying) * 18 + math.sin(T * 5 + slot) * 3 * squeeze * (1 - dying), 0, 0)
			if jolt > 0.01 then pose = K.mixPose(pose, K.Poses.Blown, jolt * 0.35) end
			-- (each bite pushes you further down into his fist - you're tiny next to his
			-- fingers, so being swallowed by them is what reads as the squeeze; it eases
			-- back to where the Funnel holds you once the screen has gone dark)
			return headCF.Position, pose, bites * 0.3 + bump * 0.35 + crush * 0.5 + K.k(t, 6.6, 8, K.E.inOutSine) * 0.1
		end)

		do
			local tp = me and me.Torso and me.Torso.Position or myPos
			local tb = headCF.Position - tp
			tb = Vector3.new(tb.X, 0, tb.Z).Unit
			lh.CFrame = CFrame.new(tp + tb * 5 + UP * 4)
			H.CrushLight.Brightness = (1.4 + bump * 4) * (1 - K.k(t, 6.2, 7.4))
		end
		...
		-- camera
		local face = headCF.Position
		local torso = me and me.Torso and me.Torso.Position or myPos
		if t < 2.5 then
			if shot ~= 1 then shot = 1 cam.cut() end
			-- level with you, from his side: the black edge of his fist climbing up your
			-- body with every bite, the realm bright behind you
			local toBoss = face - torso
			toBoss = Vector3.new(toBoss.X, 0, toBoss.Z).Unit
			local sd = toBoss:Cross(UP).Unit
			local e = K.k(t, 0, 2.5, K.E.outSine)
			local base = hcf:PointToWorldSpace(SK.fistSlot(ctx, ctx.me or 1)) -- (steady: not the struggling body)
			local p = base + toBoss * K.lerp(16, 13, e) + UP * K.lerp(8, 6.5, e) + sd * 4
			cam.go(CFrame.lookAt(p, base + UP * 0.2) * CFrame.Angles(0, 0, -bump * 0.04), K.lerp(50, 44, e) - bump * 3, dt, 10, base)
			K.shake(bump * 0.8, 0.1, 16, true)
		elseif t < 4.6 then
			if shot ~= 2 then shot = 2 cam.cut() end
			...
			local p = torso + toBoss * 13 + UP * 6 + toBoss:Cross(UP).Unit * 5
			cam.go(CFrame.lookAt(p, torso + UP * 0.5) * CFrame.Angles(0, 0, math.sin(t * 2) * 0.05), 55, dt, 10, torso)
		...
		cue("grip", t >= 0.3, function() K.sfx(K.S.DarkDrone, 0.8, 0.6) end)
		for i, at in ipairs(BITES) do
			cue("bite" .. i, t >= at, function()
				-- knuckles creaking, the air forced out of you
				K.sfx(K.S.MetalHit, 0.5 + i * 0.1, 0.34 - i * 0.02)
				K.sfx(K.S.Punch2, 0.45 + i * 0.1, 0.55)
				K.sfx(K.S.BodyFall, 0.5, 0.75 + i * 0.05)
				K.shake(0.8 + i * 0.5, 0.35, 16)
				K.kick(-2 - i, 0.3)
				if i > 1 then task.spawn(K.impact, SK.models(ctx), "W", 0.04) end
			end)
		end
...
function Ch.Funnel(ctx, t0, dur)
	...
	for i = 1, NS do
		...
		if g.Part then
			for _, b in ipairs(g.Part:GetChildren()) do
				if b:IsA("Beam") then
					-- (tinted green, not painted over: its own colours and structure stay)
					local ks = {}
					for _, k in ipairs(b.Color.Keypoints) do table.insert(ks, ColorSequenceKeypoint.new(k.Time, k.Value:Lerp(SK.GREEN, 0.5))) end
					table.insert(tint[i], { b, b.Color, b.Brightness, ColorSequence.new(ks) })
				end
			end
		end
	end
	...
	K.muffle(0.4, 0.5)
	SK.run(K, t0, dur, function(t, dt)
		local rootCF = holdRoot(ctx, t + 19) -- (the Crush's hold clock, carried on)
		...
		local palm = SK.holdPalm(ctx, t + 19, 1)
		local hcf = SK.holdFist(ctx, SB, palm, rootCF)
		-- (the fist trembles harder as the light builds inside it)
		local build = K.k(t, cutsEnd + 1.6, dur - 0.6)
		SB.Hands.Right.H.pose(0.96 - build * 0.12, -0.15, 1, 0.6 + build * 1.9, (t + 19) * 0.3)
		local surf = CFrame.new(hcf:PointToWorldSpace(SK.fistSlot(ctx, 1)))
		local headCF = SB.R.cf("Head", rootCF)
		...
		local myPos = placeInFist(ctx, hcf, t, function(slot)
			return headCF.Position, SK.gripPose(K, t + 19, slot, 0, 1 - build * 0.3), 1.5 - build * 0.5
		end)
		...
		for i = 1, NS do
			...
			if tint[i].On ~= green then
				...
				for _, e in ipairs(tint[i]) do
					e[1].Color = green and e[4] or e[2]
					e[1].Brightness = e[3] * (green and 1.15 or 1)
				end
			end
