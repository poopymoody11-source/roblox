	...
	SK.run(K, t0, dur, function(t, dt)
		...
		cue("shout", t >= HERO + 0.25, function()
			shoutText = SK.auraShout(K, "JUST WHO THE HELL\nDO YOU THINK WE ARE!", {
				...
				-- (its own font: a brush-stroke scrawl, like it was painted on in one go)
				Font = Font.new("rbxasset://fonts/families/PermanentMarker.json", Enum.FontWeight.Regular),
			})
			K.sfx(K.S.Broly, 1, 1)
...
function Ch.IAm(ctx, t0, dur)
	...
	local text
	local DROP, LAND = 3.2, 4.25 -- the fall, and the moment everyone hits the arena together
	K.lighting("Arena", 0)
	K.vignette(0.3, Color3.new(0, 0, 0), 0.3)
