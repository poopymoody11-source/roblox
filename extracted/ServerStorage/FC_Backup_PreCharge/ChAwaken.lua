	...
	SK.run(K, t0, dur, function(t, dt)
		...
		for slot, rig in pairs(ctx.rigs) do
			...
			if t < BLAST then
				...
				startCF[slot] = cf
			elseif t < LAND0 then
				-- floating up in the column, turning slowly round it
				...
				-- the dive down to the arena, and a heavy landing
				local stand = standCF(ctx, slot)
				local u = K.k(t, LAND0 + (slot - 1) * 0.05, LAND1 + (slot - 1) * 0.05, K.E.inQuad)
				local p0 = centre
				local p2 = stand.Position
				local p1 = (p0 + p2) / 2 + UP * 70
				local p = p0:Lerp(p1, u):Lerp(p1:Lerp(p2, u), u)
				if u < 1 then
					local vel = (p1:Lerp(p2, u) - p0:Lerp(p1, u))
					vel = vel.Magnitude > 1e-3 and vel.Unit or -UP
					cf = CFrame.lookAt(p, p + vel) * CFrame.Angles(math.rad(-40) * (1 - u), 0, 0)
					pose = K.mixPose(K.Poses.Launch, K.Poses.Crouch, K.k(u, 0.6, 1))
				else
					local tl = t - (LAND1 + (slot - 1) * 0.05)
					local up2 = K.k(tl, 0.5, 1.3, K.E.inOutSine)
		...
		elseif t < LAND1 + 0.2 then
			if shot ~= 6 then shot = 6 cam.cut() end
			-- diving for the arena
			local p = myPos + right * 70 + UP * 25 - look * 30
			cam.go(CFrame.lookAt(p, myPos), 55, dt, 8)
		elseif t < 10.2 then
			if shot ~= 7 then shot = 7 cam.cut() end
			-- the landing, low and close
			local st = standCF(ctx, ctx.me or 1)
			local p = st.Position + st.LookVector * 10 - UP * 1.6 + st.RightVector * 3
			cam.go(CFrame.lookAt(p, st.Position + UP * 1), 60, dt, 10)
		else
			if shot ~= 8 then shot = 8 cam.cut() end
			-- behind them: the party, blazing green, facing the titan
			local st = standCF(ctx, ctx.me or 1)
		...
		cue("dive", t >= LAND0, function() K.sfx(K.S.Whoosh, 1, 0.9) end)
		cue("land", t >= LAND1, function()
			K.sfx(K.S.Thump, 1)
			K.sfx(K.S.RockBoom, 0.9)
			K.sfx(K.S.Boom, 0.6, 1.3)
			K.shake(3, 0.8)
			for slot in pairs(ctx.rigs) do
				local p = standCF(ctx, slot).Position
				K.vfx("Shoot-01", CFrame.new(p.X, ctx.TL.ArenaCenter.Y + 0.6, p.Z) * CFrame.Angles(math.rad(90), 0, 0), A.FX, 0.9, 2, 2)
			end
		end)
		K.stream(t < LAND0 and col or ctx.TL.ArenaCenter)
	end)
...
function Ch.Shout(ctx, t0, dur)
	...
	SK.run(K, t0, dur, function(t, dt)
		...
		SB.lookAt(ctx.TL.ArenaStand(1), 1, rootCF)
		-- the party: each throws the drill up at the sky
		for slot, rig in pairs(ctx.rigs) do
			local st = standCF(ctx, slot)
			local at = 0
...
shoutSpice = function(ctx)
	...
	for slot in pairs(ctx.rigs) do mid += standCF(ctx, slot).Position n += 1 end
	mid = n > 0 and mid / n or ctx.TL.ArenaStand(1)
	local floor = Vector3.new(mid.X, ctx.TL.ArenaCenter.Y + 0.7, mid.Z)
	local cue = K.once()
...
------------------------------------------------------------------------
-- I AM: your own line, then the wide shot of the fight about to start
------------------------------------------------------------------------
function Ch.IAm(ctx, t0, dur)
	...
	local myStand = standCF(ctx, mine)
	local spice = shoutSpice(ctx)
	shrineFx(ctx, false)
	for _, rig in pairs(ctx.rigs) do
		SK.powerUp(K, rig)
	...
	lines.Parent = speed
	SK.run(K, t0, dur, function(t, dt)
		-- him: from towering over them down onto the arena, ready to fight
		local settle = K.k(t, WIDE - 0.2, dur - 0.6, K.E.inOutSine)
		local rootCF = home * CFrame.new(0, K.lerp(60, 0, settle) + math.sin(t * 1.3) * 2 * (1 - settle), K.lerp(25, 0, settle))
		ctx.BossRoot.CFrame = rootCF
		SB.reset()
		local stance = K.k(t, WIDE, WIDE + 1.2, K.E.outBack)
		SB.body({
			Waist = K.A(K.lerp(4, -10, stance), 0, 0),
			RightShoulder = K.A(-4 + stance * 20, 0, 14 + stance * 40), LeftShoulder = K.A(-4 + stance * 20, 0, -14 - stance * 40),
		...
		holdGem(ctx, SB, rootCF, t)
		local face = SB.R.cf("Head", rootCF).Position
		SB.lookAt(myStand.Position, 1, rootCF)
		-- the party: you point the Spiral Bat at him; the rest hold the pose
		for slot, rig in pairs(ctx.rigs) do
			local st = standCF(ctx, slot)
			local pose
			if slot == mine then
				...
				pose = K.mixPose(K.Poses.PointUp, K.Poses.Hero, K.k(t, WIDE, WIDE + 0.6))
			end
			rig:setCF(st)
			rig:setPose(pose)
			rig:apply()
		...
		spice(t, 0, 1 - K.k(t, WIDE + 0.5, dur - 0.3))

		----------------------------------------------------------------
		-- camera
		----------------------------------------------------------------
		local head = me and me:head() and me:head().CFrame or myStand * CFrame.new(0, 1.5, 0)
		if t < WIDE then
			if shot ~= 1 then shot = 1 cam.cut() end
			-- you, low and close, the drill of light above, him beyond
			local e = K.k(t, 0, WIDE, K.E.outSine)
			local p = head.Position + myStand.LookVector * K.lerp(6, 9, e) - UP * K.lerp(2, 1.2, e) + myStand.RightVector * K.lerp(2.2, 3.4, e)
			...
			if shot ~= 2 then shot = 2 cam.cut() end
			-- WIDE: the whole arena, the titan and the party facing off
			local e = K.k(t, WIDE, dur, K.E.outSine)
		...
		lines.ImageTransparency = (t > 0.35 and t < WIDE - 0.2) and (0.5 + 0.15 * math.sin(t * 20)) or 1
		lines.Rotation = t * 25
		...
		cue("wide", t >= WIDE, function()
			if text then text.stop() end
			K.flash(0.45, WHITE, 0.9)
			K.sfx(K.S.Whoosh, 1, 0.8)
			K.sfx(K.S.Boom, 0.9, 0.9, { Reverb = 3 })
			K.shake(2, 0.8)
		end)
		cue("roar", t >= WIDE + 0.6, function()
			K.sfx(K.S.Hell, 0.9, 0.5, { Reverb = 3 })
			K.shake(2.5, 1.4, 12)
		end)
		K.stream(t < WIDE and myStand.Position or ctx.TL.ArenaCenter)
	end)
	if text then text.stop() end
	spiceOff(ctx)
	shrineFx(ctx, true)
